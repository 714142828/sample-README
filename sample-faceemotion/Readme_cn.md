...
开�者�以将本Application部署至Atlas 200 DK上实现摄�头对视频中的人脸表情预测，共有7��能的表情，�次是生气（anger）�厌�（digust）��惧（fear）�开心（happy）�伤心（sad）�惊讶（surprised）�中性（normal）。
...
1.  <a name="li953280133816"></a>获���包。
    1.  下载压缩包方�获�。
        ...
        将[https://github.com/Atlas200dk/sample-faceemotion/tree/1-3x-0-0/](https://github.com/Atlas200dk/sample-faceemotion/tree/1-3x-0-0/)仓中的代�以Mind Studio安装用户下载至Mind Studio所在Ubuntu�务器的任�目录，例如代�存放路径为：$HOME/AscendProjects/sample-faceemotion。
...
2.  <a name="li1365682471610"></a>获�此应用中所需�的原始网络模型。
    ...
    �考[表 Head Pose Estimation中使用模型](#table144841813177)获�此应用中所用到的原始网络模型�其对应的��文件，并将其存放到Mind Studio所在Ubuntu�务器的任�目录，这两个文件必须存放到�一个目录下。例如：$HOME/models/faceemotion。
    ...
    **表 1**  Face Emotion中使用模型
    ...
    <tbody><tr id="row1710661814171"><td class="cellrowborder" valign="top" width="13.61%" headers="mcps1.2.4.1.1 "><p id="p0737128131720"><a name="p0737128131720"></a><a name="p0737128131720"></a>face_detection</p>
    </td>
    ...
    <p id="p6737102815173"><a name="p6737102815173"></a><a name="p6737102815173"></a>此模型是基于Caffe的Resnet10-SSD300模型转��的网络模型。</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="10.03%" headers="mcps1.2.4.1.2 "><p id="p19737192811710"><a name="p19737192811710"></a><a name="p19737192811710"></a>人脸表情识别网络模型。此模型是基于Caffe转��的网络模型</p>
    </td>
    <td class="cellrowborder" valign="top" width="76.36%" headers="mcps1.2.4.1.3 "><p id="p963916373173"><a name="p963916373173"></a><a name="p963916373173"></a>请�考<a href="https://github.com/Ascend-Huawei/models/tree/master/computer_vision/object_detect/face_emotion" target="_blank" rel="noopener noreferrer">https://github.com/Ascend-Huawei/models/tree/master/computer_vision/object_detect/face_emotion</a>目录中README.md下载原始网络模型文件�其对应的��文件。</p>
    </td>
    </tr>
    </tbody>
    </table>
    ...
    1. <a name="li61417158198"></a>查询当�使用的DDK版本�。
        ...
        -   使用Mind Studio工具查询。
            ...
            **图 1**  DDK版本�查询<a name="fig17553193319118"></a>  
            ![](figures/DDK版本�查询-48.png "DDK版本�查询-48")
    ...
    -   通过Mind Studio工具进行模型转�。
        ...
        2.  在弹出的**Model** **Conversion**�作界�中，进行模型转��置。
            ...
            -   Model Name填写为[表1](#table144841813177)中的模型�称。
                ...
                2.  face\_emotion模型的AIPP�置界�需�关闭AIPP。其他使用默认值。
        ...
        3.  转�时，face\_detection模型会出现[图 模型转�错误](#fig2865313121718)中所示的报错信�。
            ...
            **图 2**  模型转�错误<a name="fig2865313121718"></a>  
            ...
            ![](figures/model_facedetection_coversionfailed-49.png)
...
5.  将转�好的模型文件（.om文件）上传到[步骤1](#li953280133816)中��所在路径下的“**faceemotion/script**�目录下。
...
3.  执行deploy脚本， 进行�置�数调整�第三方库下载编译 打开Mind Studio工具的Terminal，此时默认在代�主目录下，执行如下命令在��指执行deploy脚本，进行环境部署。如[图 执行deploy脚本](#fig1733142811452)所示。

    **图 4**  执行deploy脚本<a name="fig1733142811452"></a>  
    ![](figures/执行deploy脚本-50.png "执行deploy脚本-50")
...
5.  <a name="li499911453439"></a>�动Presenter Server。
    ...
    打开Mind Studio工具的Terminal，在应用程�代�存放路径下，执行如下命令在���动Face Emotion应用的Presenter Server主程�。
    ...
    **图 5**  工程部署示�图<a name="fig999812514814"></a>  
    

    ![](figures/zh-cn_image_0219105391.jpg)

    如[图6](#fig69531305324)所示，表示presenter\_server的�务�动�功。

    **图 6**  Presenter Server进程�动<a name="fig69531305324"></a>  
    ...
    **图 7**  主页显示<a name="fig64391558352"></a>  
    ![](figures/主页显示-51.png "主页显示-51")

    Presenter Server�Mind Studio与Atlas 200 DK之间通信使用的IP地�示例如下图所示：

    **图 8**  IP地�示例<a name="fig1881532172010"></a>  
    ![](figures/IP地�示例-52.png "IP地�示例-52")
...
1.  �行Face Emotion程�。
    ...
    在Mind Studio工具的工具�中找到Run按钮，点击**Run \> Run 'sample-faceemotion'**，如[图 程�已执行示�图](#fig93931954162719)所示，�执行程�已�在开�者��行。

    **图 9**  程��行示例<a name="fig93931954162719"></a>  
...
2.  使用�动Presenter Server�务时�示的URL登录 Presenter Server 网站，详细��考[�动Presenter Server](#li499911453439)。
    ...
    等待Presenter Agent传输数�给�务端，�击“Refresh“刷新，当有数�时相应的Channel 的Status��绿色，如[图10](#fig113691556202312)所示。

    **图 10**  Presenter Server界�<a name="fig113691556202312"></a>  
    ![](figures/Presenter-Server界�-53.png "Presenter-Server界�-53")

    >![](public_sys-resources/icon-note.gif) **说明：**   
    >-   Face Emotion的Presenter Server最多支�10路Channel�时显示，�个  _presenter\_view\_app\_name_  对应一路Channel。  
...
-   **�止Face Emotion应用**
    ...
    Face Emotion应用执行�会处于�续�行状�，若��止Face Emotion应用程�，�执行如下�作。

    �击�止按钮�止Face Emotion应用程�。如[图 Face Emotion应用已�止](#fig2182182518112)所示应用程�已�止�行。

    **图 11**  Face Emotion应用已�止<a name="fig2182182518112"></a>  
...
-   **�止Presenter Server�务**
    ...
    Presenter Server�务�动�会一直处于�行状�，若想�止Face Emotion应用对应的Presenter Server�务，�执行如下�作。

    以Mind Studio安装用户在Mind Studio所在�务器中的命令行中执行如下命令查看Face Emotion应用对应的Presenter Server�务的进程。
    ...
    **ps -ef | grep presenter | grep faceemotion**
    ...
    如上所示  _7701_  �为FaceEmotion应用对应的Presenter Server�务的进程ID。
