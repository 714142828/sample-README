# 活体检测<a name="ZH-CN_TOPIC_0219080565"></a>
...
2.  <a name="li1365682471610"></a>获取此应用中所需要的原始网络模型。
    ...
    参考[表1](#table144841813177)获取此应用中所用到的原始网络模型及其对应的权重文件，并将其存放到Mind Studio所在Ubuntu服务器的任意目录，这两个文件必须存放到同一个目录下。例如：$HOME/models/faceantispoofing。
    ...
    **表 1**  Face Antispoofing中使用模型
    ...
    <td class="cellrowborder" valign="top" width="76.36%" headers="mcps1.2.4.1.3 "><p id="p1956741833120"><a name="p1956741833120"></a><a name="p1956741833120"></a>请参考<a href="https://github.com/Ascend-Huawei/models/tree/master/computer_vision/object_detect/face_detection" target="_blank" rel="noopener noreferrer">https://github.com/Ascend-Huawei/models/tree/master/computer_vision/object_detect/face_detection</a>目录中Readme_cn.md下载原始网络模型文件及其对应的权重文件。</p>
    </td>
    </tr>
    ...
    <td class="cellrowborder" valign="top" width="76.36%" headers="mcps1.2.4.1.3 "><p id="p981712263315"><a name="p981712263315"></a><a name="p981712263315"></a>请参考<a href="https://github.com/Ascend-Huawei/models/tree/master/computer_vision/object_detect/head_pose_estimation" target="_blank" rel="noopener noreferrer">https://github.com/Ascend-Huawei/models/tree/master/computer_vision/object_detect/head_pose_estimation</a>目录中Readme_cn.md下载原始网络模型文件及其对应的权重文件。</p>
    </td>
    </tr>
    </tbody>
    </table>
...
4.  将原始网络模型转换为适配昇腾AI处理器的模型。
    ...
    2.  在弹出的**Model** **Conversion**操作界面中，进行模型转换配置。
        ...
        -   Model Name填写为[表1](#table144841813177)中的模型名称。
            1.  face\_detection模型的AIPP配置界面需要将**Model Image Format**修改为BGR888\_U8。打开128*16对齐,其他使用默认值。
...
5.  将转换好的模型文件（.om文件）上传到[步骤1](#li953280133816)中源码所在路径下的“**sample-faceantispoofing/script**”目录下。
...
1.  打开对应的工程。
    ...
    **./MindStudio.sh**
    ...
    启动成功后，打开**sample-faceantispoofing**工程，如[图 打开faceantispoofing工程](#fig05481157171918)所示。
...
2.  在**src/param\_configure.conf**文件中配置相关工程信息。
    ...
    **图 4**  配置文件<a name="fig0391184062214"></a>  
...
4.  <a name="li499911453439"></a>启动Presenter Server。
    ...
    打开Mind Studio工具的Terminal，此时默认在[步骤1](#li953280133816)中的代码存放路径下，执行如下命令在后台启动Face Antispoofingon应用的Presenter Server主程序。
    ...
    **图 5**  工程部署示意图<a name="fig999812514814"></a>  
    

    ![](figures/zh-cn_image_0219084084.jpg)

    如[图6](#fig69531305324)所示，表示presenter\_server的服务启动成功。
...
1.  运行Face Antispoofing程序。
    ...
    在Mind Studio工具的工具栏中找到Run按钮，点击**Run \> Run 'sample-faceantispoofing'**，如[图 程序已执行示意图](#fig93931954162719)所示，可执行程序已经在开发者板运行。

    **图 9**  程序运行示例<a name="fig93931954162719"></a>  
...
2.  使用启动Presenter Server服务时提示的URL登录 Presenter Server 网站，详细可参考[启动Presenter Server](#li499911453439)。
    ...
    等待Presenter Agent传输数据给服务端，单击“Refresh“刷新，当有数据时相应的Channel 的Status变成绿色，如[图10](#fig113691556202312)所示。
...
-   **停止Face Antispoofing应用**
    ...
    单击停止按钮停止Face Antispoofing应用程序。如[图 Face Antispoofingon应用已停止](#fig2182182518112)所示应用程序已停止运行。

    **图 11**  Face Antispoofingon应用已停止<a name="fig2182182518112"></a>  
