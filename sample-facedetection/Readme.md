...
1.  <a name="li953280133816"></a>获取源码包。
    ...
    2.  命令行使用git命令方式获取。
        ...
        **git clone https://gitee.com/Atlas200DK/sample-facedetection.git --branch 1.3x.0.0**

2.  <a name="li1365682471610"></a>获取此应用中所需要的原始网络模型。

    参考[表1](#table144841813177)获取此应用中所用到的原始网络模型及其对应的权重文件，并将其存放到Mind Studio所在Ubuntu服务器的任意目录，这两个文件必须存放到同一个目录下。例如：$HOME/models/facedetection。
...
3.  以Mind Studio安装用户登录Mind Studio所在Ubuntu服务器，确定当前使用的DDK版本号并设置环境变量DDK\_HOME，tools\_version，LD\_LIBRARY\_PATH。
    1.  <a name="li61417158198"></a>查询当前使用的DDK版本号。
        ...
        -   使用Mind Studio工具查询。
            ...
            在Mind Studio工程界面依次选择“File \> Settings \> System Settings \> Ascend DDK“，弹出如[图 DDK版本号查询](#fig17553193319118)所示界面。
...
4.  将原始网络模型转换为适配昇腾AI处理器的模型，模型转换有Mind Studio工具转换和命令行转换两种方式。
    -   通过Mind Studio工具进行模型转换。
        ...
        2.  在弹出的**Model** **Conversion**操作界面中，进行模型转换配置，如[图 face\_detection模型转换配置](#fig206931026131712)所示。

            **图 2**  face\_detection模型转换配置<a name="fig206931026131712"></a>  
            

            ![](figures/models_facedetection1.png)

            -   Model File选择[步骤2](#li1365682471610)中下载的模型文件，此时会自动匹配到权重文件并填写在Weight File中。
            -   Model Name填写为[表1](#table144841813177)中的模型名称：**face\_detection**。

        3.  单击**Next**，进入Nodes配置界面。

            **图 3**  Nodes配置示例<a name="fig3754173017185"></a>  
        ...
        5.  单击**Next**，进入AIPP配置界面。需要将**Input Image Size\[W\]\[H\]**分别修改为384,304， 此处需要做128\*16对齐。需要将**Model Image Format**修改为BGR888\_U8。其他使用默认值。如[图 AIPP配置](#fig1682055223010)所示

            **图 4**  AIPP配置<a name="fig1682055223010"></a>  
            ![](figures/AIPP配置.png "AIPP配置")

        6.  单击**Finish**，开始进行模型转换。

            转换时，会出现[图 模型转换错误](#fig2865313121718)中所示的报错信息。

            **图 5**  模型转换错误<a name="fig2865313121718"></a>  
...
5.  将转换好的模型文件（.om文件）上传到[步骤1](#li953280133816)中源码所在路径下的“**sample-facedetection/script**”目录下。
...
1.  打开对应的工程。
    ...
    **./MindStudio.sh**
    ...
    启动成功后，打开**sample-facedetection**工程，如[图 打开facedetection工程](#fig05481157171918)所示。
...
2.  在**src/param\_configure.conf**文件中配置相关工程信息。
    ...
    **图 7**  配置文件<a name="fig0391184062214"></a>  
...
3.  执行deploy脚本， 进行配置参数调整及第三方库下载编译 打开Mind Studio工具的Terminal，此时默认在代码主目录下，执行如下命令在后台指执行deploy脚本，进行环境部署。如[图 执行deploy脚本](#fig107831626101910)所示。
...
4.  开始编译，打开Mindstudio工具，在工具栏中点击**Build \> Build \> Build-Configuration**。如[图 编译操作及生成文件](#fig1625447397)所示，会在目录下生成build和run文件夹。

    **图 9**  编译操作及生成文件<a name="fig1625447397"></a>  
...
5.  启动Presenter Server。<a name="li499911453439"></a>

    打开Mind Studio工具的Terminal，在应用代码存放路径下，执行如下命令在后台启动Face Detection应用的Presenter Server主程序。如[图 启动PresenterServer](#fig423515251067)所示。

    **bash run\_present\_server.sh**

    **图 10**  启动PresenterServer<a name="fig423515251067"></a>  
    ...
    **图 11**  工程部署示意图<a name="fig999812514814"></a>  
    

    ![](figures/face_detection_presentserver2.png)

    如[图12](#fig69531305324)所示，表示presenter\_server的服务启动成功。

    **图 12**  Presenter Server进程启动<a name="fig69531305324"></a>  
    ...
    **图 14**  IP地址示例<a name="fig1881532172010"></a>  
...
1.  运行Face Detection程序。
    ...
    在Mind Studio工具的工具栏中找到Run按钮，点击**Run \> Run 'sample-facedetection'**，如[图 程序已执行示意图](#fig93931954162719)所示，可执行程序已经在开发者板运行。

    **图 15**  程序运行示例<a name="fig93931954162719"></a>  
...
2.  使用启动Presenter Server服务时提示的URL登录 Presenter Server 网站，详细可参考[启动Presenter Server](#li499911453439)。
    ...
    **图 16**  Presenter Server界面<a name="fig113691556202312"></a>  
...
-   **停止Face Detection应用**
    ...
    单击[图 停止Face Detection应用](#fig14326454172518)所示的停止按钮停止Face Detection应用程序。

    **图 17**  停止Face Detection应用<a name="fig14326454172518"></a>  
    

    ![](figures/face_detection_stopping.png)

    如[图 Face Detection应用已停止](#fig2182182518112)所示应用程序已停止运行

    **图 18**  Face Detection应用已停止<a name="fig2182182518112"></a>  
